...
- **保留**：起作用的是豁免——**被存活记忆引用的会话永不删除**（记忆不得比支撑
  它的原话活得更久）；随 jobs 清理同批执行。90 天窗口是次要的一半，且**至今
  从未触发过一次删除**：`evidence.ref` 记的是整个会话 id，一条记忆即钉住其所
  在会话的全部回合，实测（2026-09-01，9 个真实库中 8 个非空库的全部 L0 行）
  豁免集恒等于全集；时间条件本身也要到 2026-11-21 才首次可能被满足。故
  **在此明确登记：L0 的实际生命周期无上界**，这是已知取舍而非隐含承诺——
  90 天不是一个会兑现的删除期限。窗口保留的理由是它是唯一按"行有多老"生效的
  保护，覆盖 capture 与 extract 之间的必然时间窗（详见
  `store/conversations.ts` 的 `pruneConversations` 文档注释）。

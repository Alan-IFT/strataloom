...
4×4**。用更少机制承载同一层/类是对的（如 L2/L3 靠加宽 `derived` 而非新增
表）；因机制复杂而砍掉一层/一类是错的。差距只能靠实现关闭，不能靠改小目标
关闭；确实受阻时须开 ADR 记录具体条件，并保留在目标形态中。
当前 L2/L3 与 Coding Memory 尚缺，边界与验收见 `docs/design/4x4-memory.md`。

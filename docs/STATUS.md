...
| 2 | **合规派生层可因换行密度突破预算** | 6 块全部 ≤ `ROLLUP_TARGET_CHARS` 时，行长 ≤6 字符即 >1300（最高 1.5×），运行时静默丢块 | 该暴露 `constants.ts` 已在 `DERIVED_WORST_LINE_CHARS` 上登记。真正的修法是把 `parseScenarios` 的约束从**字符数**改为**渲染后 token 数**——即「量到最外面那把尺子」。属独立评估，会移动写入路径 |

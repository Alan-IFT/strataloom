...
**阶段 3：L3 Core/Persona**。触发条件**已定**，可直接动手：
见 [`decisions/0004-l3-rebuild-trigger.md`](decisions/0004-l3-rebuild-trigger.md)
与设计文档阶段 3。

要点：schema 无需再改（`LAYER.PERSONA` 已被 v7 接受）；触发条件是「画像缺失
或模型判定过时」，在既有维护轮里问一句即可——**不加阈值/计数器/列/job kind**；
D9 保持不变，画像被删后由下一轮重建。

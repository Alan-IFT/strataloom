...
> 最后更新：2026-09-02 · **v0.4.6 已发布**：给注入包的**最外层容器**（spec §4.2
> 的 1400）补上加载期护栏。**产品行为零变化、落库字节零变化**——本轮只新增护栏
> 与测试，不动任何运行时路径。
> **每次工作结束时更新本页**，它是新会话的唯一入口。
>
> 版本序（**取数时刻 2026-09-02 01:23**）：装机版本 **0.4.5**，插件落盘
> **2026-09-02 01:22:55**，进程启动 **2026-09-02 01:23:02**（PID 3057476）——
> 进程晚于插件，故 **0.4.5 确已在跑**（这是 0.4.3–0.4.5 三版首次真正生效）。
> **v0.4.6 已发布、尚未安装**，判据同下节。
>
> **要让 0.4.6 生效，需先安装再重启**（`INSTALL.md` 的 remove-then-add，
> 然后重启 harness）。**但本轮的交付物是加载期护栏，它的「生效」判据不是运行时
> 行为**——护栏在 `import` 时求值，装上即生效；它今天**不触发**（1361 ≤ 1400），
> 所以「装了也看不出变化」是**预期**，不是没装上。
>
...
| j | **`queryInjectionRows` 的派生分支无 LIMIT，护栏靠写路径而非读路径兜底** | SQL 层塞 150 行实测**会全量返回**；但合法写路径封顶 **7**（`parseScenarios` 切 `ROLLUP_MAX_SCENARIOS`、提交前 `DELETE ... WHERE derived != RAW`、persona 只留 1 行），计入 v6 遗留 SUMMARY 最坏 **8** | 今日安全**有 5 倍余量**：打破容器需 **E ≥ 197**（通用界 `ceil((H+1+4B+E)/4)`），而可达 E 最大 40。属**读路径依赖写路径的不变量**，与 v0.4.4 待办 4 是同一条 LIMIT。加 `LIMIT` 可同时了结两条 |

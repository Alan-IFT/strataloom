...
> 最后更新：2026-09-02 · **v0.4.8 已发布**：给派生层补上**行数**守卫
> （`ROLLUP_MAX_SCENARIOS + 1 ≤ INJECT_TOP_N`），并**更正待办 k 的失真描述**。
> **产品行为零变化**——只新增一个加载期 `if/throw` 与注释。本轮结项的是
> **上一轮登记的待办 k**，但**结项方式与当时登记的判据相反**（见下节）。
> **每次工作结束时更新本页**，它是新会话的唯一入口。
>
> 版本序（**取数时刻 2026-09-02 09:17**）：装机版本 **0.4.6**，插件落盘
> **2026-09-02 09:16:45**，进程启动 **2026-09-02 09:16:57**（PID 1151655）——
> 进程晚于插件，故 **0.4.6 确已在跑**。
> **v0.4.7 已发布、尚未安装**，判据同下节。
>
> **本轮交付物今天不改变任何运行时行为**：真实库派生行最多 6 条，而上限是 20，
> 故 `LIMIT` **今天一行都截不掉**。它把「最坏条目数」从**存储内容的属性**变成
> **常量**，即把加载期护栏一直在假设、却无人执行的那个前提**变成机制**。
...
| p | **`queryInjectionRows` 的派生分支不过滤 `provenance`，回退分支过滤** | 实测：往派生层插 `provenance='tool-output'` / `'subagent'` 的行，**两条都被注入**；而同样 provenance 的 **raw 行被 `queryInjectableSet` 挡掉** | **今天不可达**：唯二的派生写者都硬编码 `provenance='derived'`。但这是**读路径信任写路径不变量**的又一处——正是 v0.4.7 那条 LIMIT 修的同一种形状，且**当时只修了行数这一维**。§2.3 的「`tool-output`/`subagent` 永不默认注入」在派生分支上**没有执行点**。判据：任何让派生行携带非 `derived` provenance 的改动，必须先在此加过滤 |

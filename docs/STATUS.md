...
## 🟡 验收进行中：0.3.5/0.3.6 刚开始生效，判据尚未满足
...
**但三条判据都还没满足，因为它们都需要新代码先跑出数据**——采集在
`agent/turn-stopping` 触发，派生层重建在周期性维护轮（`CLEANUP_INTERVAL_MS`
= 6 小时）。**「刚重启所以还没有」与「重启了但没生效」在数据上可分，下面
给出基线，下次开工按它对照，不要凭印象下结论**（上一轮正是在这里误判过）。
...
**验收判据**（三条都要新数据才能判；查到「无新数据」时结论是「还没跑」，
不是「没生效」）：

1. **新写入**的 L0 出现 `tool-call:` 行 → 该窗口孤儿率从 100% 下降；
2. **那条 677 字符的画像被重写到 600 以内** —— 它是「未生效」的现场物证，
   也是「已生效」的天然判据。触发链：任何 personal raw 写入 → D9 触发器删
   画像 → 下一轮维护重建（≤6 小时）；
3. **新产生**的 L2 场景块 body ≤ `ROLLUP_TARGET_CHARS = 620`。现存 17 块
   均为旧代码产出，会在各自库触发 rebuild 时被整批替换。

**在这三条满足之前，「XX 已修复」只对代码成立，对运行中的系统不成立。**

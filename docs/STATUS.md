...
> 最后更新：2026-08-31 · **v0.3.7 已发布并安装，等待重启**（schema v9，无迁移）
...
stat -c '%y' $P/lib/pipeline/extract.js               # 插件落盘时刻
...
#### ⏳ v0.3.7 验收判据（**待重启后求值**，2026-08-31 22:51 装机）

已发布 v0.3.7 并装机（装机代码已核对：`slice(0, WORST_SOURCE_SEQS)` 与
逐段 `capEventText` 在位，`join` 后的旧 `.slice()` 已消失）。**但当时跑着的
进程启动于 22:46:35，早于插件落盘，故尚未生效。**

判据是**肯定式**的，且**只能由新写入的行回答**——历史 241 条全是旧写者产物：

```sql
-- 重启后新写入、且引用了多个来源的 evidence
SELECT length(excerpt), excerpt FROM evidence
WHERE excerpt IS NOT NULL AND excerpt LIKE '%' || char(10) || '---' || char(10) || '%';
```

1. **新写入的多段 excerpt 不再恰好 400 字符**，且每段各自受限
   （每段引文 ≤ 401 = 400 + 省略号），而非多段共享 400。
2. **段数 ≤ `WORST_SOURCE_SEQS`（10）**。

> **读数陷阱**：查到「没有新行」时，结论是**「还没跑到」**，不是「没生效」。
> extract 在 `agent/turn-stopping` 触发，需要真实会话产生新 turn。
> 先按本页顶部那条命令确认进程确实比插件新，再看数据。

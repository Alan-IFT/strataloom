...
> 🆕 **最后更新：2026-09-07（第九节）：ADR 0015 §四·十二 那个「唯一现在就该做的事」已补上防护。**
> **产品逻辑零改动**（本轮只加了一条测试和三处注释），仍为 v0.5.2，**不发版**。
>
> ✅ **`overturnRate` 的 `archived` 项不再零防护。**
> 新测试 `overturnRate counts an archived procedure as an overturn, not as a row that vanished`
> （`test/layers.test.mjs`）钉住了这一项，fixture 由**两次真实 `runReconcileJob`** 产出
> （active=3 / superseded=4 / archived=1 / rejected=3 → `0.4`；第二次退休后 4/4/2/3 → `0.429`）。
> **七个竞争公式在这个 fixture 上给出七个互不相同的数**：
> `0.4`（正确）/ `0.25`（分子分母同删 archived，即本次要防的那个变异）/ `0.2` / `0.5` /
> `0.625`（旧混同式）/ `0.571`（谓词判反）/ `0.333`（分母把 `active` 误写成 `superseded`）。
> **六个变异已实跑验证**（第七个 0.625 仅算术），主变异 V1 在改动前跑出的正是未动的基线
> `327/320/6`——**它此前确实完全不可见**。
>
> ⭐ **方案审查捕到一个我自己没看出来的盲点**：我最初的 fixture 只放 2 条 drop 候选，
> 导致 `active == superseded == 3`，于是「分母把 `active` 误写成 `superseded`」这个
> copy-paste 型变异**读出同一个 0.4、不会变红**。第三条 drop 候选 `c-dup3` 就是为打破这个等值
> 而存在的，测试头注里写明了「不要把它简化掉」。
>
> 🔴 **本轮更正一个 ADR 自己的读数错误**：ADR 0015 §四·十二与本文件第八节都写
> 「删掉 `archived` 项会让 **13** 条真实退休消失」——**正确是 12**。
> 删掉该项丢掉的恰好是 `status='archived'` 的行，实测九库共 **12** 条
> （ADR 0015 §四·八 更正 ① 的实测分解：`procedure/archived:12, procedure/superseded:1`）；
> 第 13 条是 `propose` 写出的那条 `procedure/superseded`，**状态是 `superseded`，删掉 archived 项它不受影响**。
> ⚠️ **「13」与 ADR 自己 §四·八 的更正互相矛盾**——一个已被更正过的读数，在同一份 ADR 的另一节里活了下来。
> 代码里现在**不再独立复述这个数字**，而是锚定到 `metrics.ts` 已有的那次测量（`38 superseded + 12 archived`），
> 使两处不可能各自漂移。
>
> ⚠️ **另一处被审查挡下的注释错误**：返工前的注释把本机基线的六条平台性失败枚举成了五条，
> 且把 layers 那条清扫钩子的级联来源归给了 symlink（实际来源是被漏写的 `share projection` 那条）。
> **已删掉整份枚举**，改为只声明「基线含六条平台性失败、与本改动无关、跑一次未改动的套件即可自查」
> ——一份错的枚举比没有枚举更糟，且那份清单本就是 Windows 特有的，Linux 读者对着它必然对不上号。
>
> 🟡 **本机测量口径**：Windows 基线 `327/320/6 (+1 skipped)`，改动后 `328/321/6`。
> 那 6 条全是平台性失败（symlink EPERM / 并发开库 / 不可用库 / share projection / 两条清扫钩子级联），
> **本机拿不到全绿基线**，所以注释里一律用「BASELINE + 2」这种增量表述，不写「其余 N 条全绿」。
> 连跑四次无抖动，`layers.test.mjs` 单文件跑亦通过。
>
> ⬇️ 以下 2026-09-07（第八节）及更早各节仍然有效
> （**其中第八节「13 条」一处已由本节更正为 12**）。

---

> 🆕 **最后更新：2026-09-07（第八节）：v0.5.2 已真正跑起来；四项待裁定全部裁为「不做」。**
> **产品代码零改动**（本轮是审计与裁定），仍为 v0.5.2。**详见 [ADR 0015 §四·十一/十二](decisions/0015-converge-at-the-write-and-scope-invalidation-to-what-is-visible.md)。**
>
> ✅ **「装上了 = 跑起来了」这次成立**（三轮以来第一次）：
> 已安装 v0.5.2、`TARGET_USER_VERSION = 12`、A4 豁免已从安装构建中移除、**9 库全部迁移到 v12**、
> 生产库里的 D9 触发器实测已带 `OLD ∪ NEW` 源集合谓词。
>
> **四项裁定（四路 agent 并行审计，主 agent 逐项复现）：**
> ⛔ **B4-c `retrievedRate` → 删掉这个指标，不要修它**。唯一消费者是一条 `logger.info`；
> 声明服务的 decay **已完整落地且不读它**（`decay.ts` 的 active 计数自己写着 `AND derived = 0`）；
> **换成常量 1 仍 325/325 全绿**。且**修口径治不了根本问题**——
> `retrieved` 是累计量、`active` 是瞬时快照，**除出来本就不是「率」**
> （实测 9/9 库自派生层诞生以来 recall 调用数 = 0，三库读数的分子来自 8 月、分母来自 9 月）。
> 外部：15 个项目 **14 个不做**，唯一做的那个自己的 issue 说它坏了。
> ⛔ **C5b → 不上调，先加观测**。⭐ **我基于「闲置 784 tok」的推论被实测反转**：
> `kindsFed` 在 5 个库上**零方差**（全是 2）而 scenario 产出 2–6，**零方差变量不可能解释变异**；
> 真正单调相关的是 `fedRows`，而 **C5b 不增加行数反而减少** ⇒ 会让产出更少。
> 且闲置有一半是**结构性**的（repo 库永远拿不到 L3 画像的 171 tok 份额）。
> ⭐ **「C5a 让用户看到更少记忆」不成立**：内容判读显示 RAW 分支前 6 条里 5 条在复述同一条工作流，
> 而 DERIVED 3 条覆盖三个不同主题簇——**更少 token、更多主题；摘要在这里是去重不是损失**。
> ⛔ **`archived` 不一致 → 不单独修**，归入未来 v13「三态一次定清」。全仓**无消费者**
> （两状态在所有读路径行为逐条相同）；且 (a)「让 propose 也写 archived」**最不该选**
> ——它把无消费者的语义断言扩大到第二个写入点。外部：主流是**合并甚至无状态**。
> ⛔ **存量清理 → 不清理**。⭐ **我上轮的收益估计被证伪两处**：实测清理后
> **条目数 8→6（减少）、token +47（不降反升）**。更致命的是——
> **不存在「等派生层缺席时收益才体现」这回事，因为清理自己制造缺席**：
> 仅 supersede 1 条 human preference 就让 `derived 2→0`；14 条待清理中只有 1 条能安静清掉。
> **「派生层活着 ⇒ raw 不进包」是读的性质，而清理是写——前提在被使用的那一刻就失效了。**
> 外部：**mem0 v0.2.13 处理同型事故的官方处置就是「不追溯 + 告知用户手动删」**；
> cognee 给出判据——**当且仅当「重复」能被确定性主键定义时才可无人确认地批量清理**，本例落在反面；
> graphiti #1728 人工抽检**误判 75% 且静默**。
>
> 🔴 **唯一现在就该做的事**（独立于四项裁定）：`overturnRate` 漏计 `archived` 的变异
> **325/325 全绿存活**——生产上会让 13 条真实退休从信任指标消失。**这是唯一防护真实数值损害的缺口。**
>
> ⚠️ **本轮三起纪律事故已登记**（agent 污染仓库 / 子代理擅自写库 / 外部案例尺度不匹配），见 ADR §四·十二。
>
> ⬇️ 以下 2026-09-07（第七节）及更早各节仍然有效。

---

> **2026-09-07（第七节）：ADR 0015 五步全部走完；第 5 步重估结论是「三项都暂不实施」。**
> **产品代码零改动**（重估是纯评估），仍为 v0.5.2。
>
> ⛔ **本轮第一件事推翻了「已发版」的含义：前三轮的改动一次都没跑起来。**
> ```
> 9 个生产库全部 user_version = 11        ← C5a 的 v12 迁移从未生效
> 已安装插件 = v0.4.17（lib 日期 9-05），TARGET_USER_VERSION = 11
> 安装的 reconcile.js 仍含 kind !== 'preference'   ← 即 pre-A4
> GitHub latest release 一直停在 v0.4.17（附件 9-04）
> ```
> **根因**：安装源是 GitHub release 附件，而我前三轮只做了「改版本号 + commit + push」，
> **从未打 tag、从未跑仓库里现成的 `scripts/release.sh`**。
> **「Release 0.5.0」这个 commit 标题是真话，但它让我误以为发布完成了**
> ——改版本号是**声明**，跑发布脚本才是**交付**。
> ✅ **已补发 v0.5.2 并验证附件内容**（不是只看命令成功）：拉回 latest 附件实测
> `version 0.5.2`、`TARGET_USER_VERSION = 12`、`A4 豁免已移除`。
> （`npm pack` 会撞 npm 缓存 root 权限，用 `npm_config_cache=/tmp/npmcache` 绕开——本仓已登记的做法。）
> ⚠️ **仍需你执行 `remove` + `add` 重装并重启 harness**，改动才会在你的库上生效
> （`INSTALL.md` 明确 plain `add` 是 no-op，pnpm 固定已解析的 URL）。
>
> **第 5 步重估结论（全部基于反事实模拟，因为生产库仍是 pre-C5a/pre-A4 行为）**：
> ⛔ **C4 删掉 L2 → 否决，且比当时更坚决**：A4 反事实下仍 **6/9 库超预算**
> （4 个聚类阈值 × 2 种幸存者选法 = 8 种组合**无一格改变**）；**删光全部 preference 仍 5/6 超预算**
> （`global` 8.35×、`5ed2b4d2` 5.49×）。⭐ **A4 消除复述，L2 应对体量——不是同一个问题**；
> 最强证据是 3 个超预算库**连一个多行族都不存在**。C5a 又降低了 L2 的单位成本。
> 🟡 **C5b 分层取样 → 确认为主修，但本轮不做**：⭐ **A4 完全没改变 `kinds_seen`**
> （6 个库逐格相同：2/4、2/3、1/2 全不变），而 C5b 把它们提到满覆盖——
> **A4 减少族内重复，C5b 修 kind 间窗口争抢，不是同一件事**。
> ✅ **不必发明第二种排序**：按 kind 稳定分区后轮转，每个 kind 内部逐字保持 SQL `ORDER BY`，
> 新增的是**配额**不是排序（与 Memobase「先分组、只压超限组」同构）。
> ⛔ **C2 → 从「可选」降级为两种形态均否决**，且**C 组对它的归因是错的**：
> C 组说「6 条同族 preference 被塞回包」，实测另外 3 个库塞回的**全是 coding、零 preference、
> 重复率 100%、A4 前后逐格相同**。真正成因是结构性的——**rollup 输入与 C2 填充
> 从同一个有序列表的同一端取用**。**这是本轮第四次「单库读数当全样本」，
> 而这次它被固化成了因果解释**：按该归因设计的修法会去修「同族」，而真正要修的是「同一端」。
> ⚠️ **口径更正**：本文与审计文全文引用的「**4 个**超预算库」**实测已是 6 个**
> （用生产 `packetOverflows` 复现 6/9）。**该数被 C4 的否决论证直接引用。**
> 🟡 **`kinds_seen` 是比 `coverage%` 更好的尺子，而全仓没有任何地方在测它**
> ——覆盖率在 `5ed2b4d2` 上 A4 后**下降却是改善**（喂 9 条同族的 28.6% 不如喂 4 个 kind 的 22.9%）。
>
> **下一轮待办（按优先级）**：
> **P0（非代码）用户重装 + 重启** —— 它是其余一切的前提；
> **P1 B4-c `retrievedRate`**（唯一今天就在输出错误数字的缺陷，跨 9 库 `usageOnDerived = 0`；
> ⛔ **这句处方已被实测证伪，见下节更正**：原写「分子的行物理上已不存在，只能摘分母」
> ——**只摘分母会算出 1.25 的「率」**，因为 C5a 生效后派生行**可以同时活着且已被 recall 命中**
> （`queryRecallRows` 不过滤 derived）。**正解与 B4-b′ 同形：两侧同摘。**）；
> **P2 `propose`/`reconcile` 对 `archived` 不一致**（12 条走 reconcile 记 archived，1 条走 propose 记 superseded）；
> **P3 C5b**（须等 P0）；**P4 存量 preference 清理**（须你确认，且实测**不改变 `packetOverflows`**，收益低于当时预期）。
>
> ⬇️ 以下 2026-09-07（第六节）及更早各节仍然有效。

---

> **2026-09-07（第六节，已发版 **v0.5.2**）：B4-b 被打回，改做 B4-b′，324 → 325 全绿。**
...
> **写入面无人守**；而发版面守卫是**时间窗，两个方向都会错**（已登记的假红实证 ＋ 会漏掉真泄漏）。
> 四个探针**实测真的活在 `lib/` 里**（1ms 轮询真实全量，4/4 全部观测到）。
> 修法：一个获取点 `withLibProbe`，副本放 `lib/` 的**同级**目录；**六个站点全部收敛**
> （全树 `cpSync` 真实拷贝只剩 `helpers.mjs:798`）。
> ⛔ **两条守卫，实测互相杀不掉对方的变异，故两条都要**：写入处断言（杀得掉**计算出来的**目的地）
> ＋ 目的地感知的静态检查（杀得掉绕过 `dir` 的另一处写）。承重性双向实测：各自删掉即**全绿复活**。
> ⛔ **本轮最重要的方法论收获**：「把探针目录移进 `lib/`」这个变异体**证明不了断言承重**——
> 删掉断言它照样红，因为 **Node 的 `cpSync` 自己就拒绝**「拷到自身子目录」。
> **执行 agent 自己发现并拒绝照抄 lead 的说法**，改用绕开该 backstop 的忠实变异体才真正隔离出断言。
> ⛔ **三个候选守卫实测否决**：照抄上一轮 tempRoot 的 `after()` 钩子（采样竞态，且**点名无辜文件**）、
> 朴素 grep（**对正确代码开火**且漏真变异）、ESM 里 patch `node:fs`（**结构上不可能**，命名空间冻结）。
> 🆕 **QA 抓到两个真缺陷，均已修**：`symlinkSync`／`linkSync` 在 `WRITERS` 里却不在 `DEST_INDEX` 里，
> **读错参数位、双向都错**（漏真泄漏 ＋ 对正确代码误报）——**正是该设计自称要避免的错误**；
> `mkdtempSync` 缺席，planted 真泄漏 **`310/310/0` 全绿两守卫全哑**。
> ⚠️ **根因是 `WRITERS` 无完备性压力（12 个里 9 个可静默删除），且表驱动本身不够**——
> 自检遍历该表时**删一行会连它自己的用例一起删掉**，真正起作用的是**独立的成员资格断言**。
> 测试 309 → **310 / 0 fail**（名称 diff **+1、删 0、改名 0**）。
> ⛔ **明确不发版**：只动 `test/`，而 `files` 不含测试文件；**tarball sha256 `4901c38b…` 与已发布的
> v0.4.17 完全相同**、59 条目、探针匹配 0。**没有可交付的字节变化，就没有版本。**
> **每次工作结束时更新本页**，它是新会话的唯一入口。
>
> ⬇️ 以下 2026-09-07（第一节，ADR 0014）及更早各节仍然有效。

---

> 最后更新：2026-09-06（第二节） · **本轮不发版，版本仍为 v0.4.17**（纯测试基建，产品代码零改动）：
> **`tempRoot()` 把它造出来的目录的唯一句柄丢掉了，于是全量每跑泄漏 213 个 `/tmp` 目录。**
> ⛔ **判别式不是「测试有没有清理」，而是「外层句柄被留住还是被丢掉」**：
> `store.test.mjs` 调 `tempRoot()` **20 次、泄漏 0**（它写 `const root = tempRoot()` 再 `cleanup(root)`）；
> 而 `join(tempRoot(), 'repo')`／`cwd: tempRoot()` 这类调用**只留内层路径**，
> 外层 `/tmp/strataloom-test-XXXXXX` **从未被绑定到任何变量**——
> 不是调用点疏忽清理，是**它根本无法命名要清理的东西**。
> ⛔ **本轮证伪并改正本页两处旧读数「每跑 delta=9」（原文保留在下方，勿改）**：
> 9 **是一次正确的测量，但量的是错误的范围**——它 ＝ e2e(7)＋command(2)＋lifecycle(0)，
> 恰是 v0.4.17 那轮的三文件范围（`grep -ln "ctx.plugin(memoryPlugin" test/*.mjs`）。
> 本 agent 单跑这三个文件复现：**delta ＝ 9，分毫不差**。**它不是 23 倍的测量错误**，
> 缺的是**范围标注**：「每跑」读起来就是全量，于是这条泄漏被 🟢 归档了三轮。
> 真实全量读数：**每跑 213**，独立 3 跑完全一致，且 3 次均 309/309/0。
> 修法**只动 `test/helpers.mjs` 一个文件、调用点零改动**：在唯一获取点登记，
> 用 `node:test` 的**文件级 `after()`** 兜底删除。**213 → 0**（全量 3 跑、逐文件 16/16 全部 delta=0）。
> ⛔ **必须是 `after()` 而不是 `process.on('exit')`**，这是实测不是偏好：
> 本仓已登记的挂死形态（失败断言 ＋ 插件自持 30s `ctx.interval` ⇒ 文件不退出 ⇒ 外部 kill，RC=124）
> 下，`exit` 处理器**根本不运行**——两个仅此一处不同的 helper 实测
> **`after()` rc=124 leaked=0／`exit` rc=124 leaked=1**。
> 🟡 **明说不粉饰：SIGKILL 打在测试执行途中仍泄漏 1**，任何进程内方案都挡不住（实测）。
> 🆕 **回归守卫写成第二个 `after()` 钩子，不是 `test()`**：文件级 `after()` 在所有 test 之后跑，
> 写成 `test()` 会**读到清扫之前的状态、在已修好的代码上翻红**。跨模块 `after` 为 FIFO（实测）。
> **守卫看着红过**：临时停掉清扫 ⇒ **`tests 316 / pass 309 / fail 7`**，
> 七条失败各点名一个文件，计数 102/83/11/7/6/2/2 **恰好合计 213**。
> ⛔ **禁止再写「还剩约 26 次全量跑就耗尽 inode」**：那个读数取时为真，但泄漏的
> **33.2 万** 个目录**已被回收**（inode 使用率 99% → **10%**，可用 ~1409 万）。
> **稳定且仍为真的量是：每次全量跑 213 个目录／约 9,270 个 inode，无上界增长**，
> 且 `/tmp` 与 `/` **同一文件系统**（实测 fsid 相同、同为 `/dev/nvme0n1p2`）。
> **回收与代码修复是两件可分开的事**，本轮两件都做了。
> 🆕 **代码审查打回（PASS WITH FIXES），三项必修均已修**：
> ①一处**假注释**（helpers 写「其余 10 个文件」，真值 **9**；`16 − 7 = 9`，
> **同一事实在本页逐文件表里写的是 9、是对的**——一个事实两处书写、其中一处错）；
> ②③**两个变异全绿存活**：删 `created.add(dir)`、或把守卫判据改成读 `live` 而不问磁盘，
> **均 `309/309/0`、rc=0、delta=0**——清扫照常工作，**只是探测器被摘掉了**。
> 已补一条**端到端阴性对照**（造真目录 → 断言守卫自己的谓词报出它 → 删掉 → 断言它安静），
> 两个变异现均 `rc=1 / fail 14`。⛔ **审查提议的候选断言经实测两个变异一个也杀不掉**
> （它对正确代码也从不开火：钩子 FIFO，清扫已 `live.clear()`，故谓词两个方向都不可满足），
> **已拒绝采纳并连同读数登记为证伪**——审查明说该候选未经验证并要我自测，自测结论即此。
> 🆕 **QA 再打回两条，均已修**：
> ④**又一条假注释（「说了半句真话」的形状）**：原文在「为什么不用 `exit`」标题下写
> 「`exit` 确实覆盖……`process.exit()`」，**关于 `exit` 的部分是真的**，却诱导读者以为
> **`after()` 是 `exit` 的超集**。实测**二者互有胜负**：`after()` 赢 RC=124 挂死（`exit` 泄漏 1），
> **`exit` 赢 `process.exit()` 中途退出（`after()` 泄漏 1）**。仍选 `after()`，
> 理由是本仓性质而非普适优劣——RC=124 有文档且真实发生过，而
> **`process.exit()` 在本包根本不存在**（本 agent 自己 grep：全包唯一命中是那段注释本身）。
> 残留缺口清单也从「只写 SIGKILL」补齐为 **SIGKILL／SIGTERM／`process.exit()` 三种各泄漏 1**。
> ⑤**组合变异真盲区**：单独无害的变异两两组合后**全绿 `309/309/0` 却泄漏 213**
> （C1 ＝ 去 `created.add` ＋ 清扫提前 return；C2 ＝ 清扫只 `live.clear()` ＋ 守卫读 `live`）。
> QA 要求我加完自检后**实测并如实报告是否覆盖**。
> ⛔ 🆕 **复审第三次打回，抓到本轮第四次「为真实断言编造错误理由」，且这次是我错**：
> 我上报的「D3／C2 已转红」**取自一个比复审更弱的变异体**——我把谓词改成裸 `[...live]`
> （丢掉 `existsSync` 过滤），那个会死；**复审保留过滤的 `[...live].filter(existsSync)` 才是现实形态，
> 它完整存活 `rc=0 309/0`，C2 更是全绿泄漏 213**。我据弱变异体外推，写下了
> 「步骤 4 是唯一抓得住谓词读错账本的那一步」——**该句在数学上不可能为真**：
> 步骤 4 跑在 `rmSync` 之后，`existsSync` 为 false，**两个谓词都被过滤掉、都通过**。
> **修法（复审给的形状，我实测后采用）**：新增**步骤 B**——「从 `live` 移除但**不删磁盘**，
> 断言谓词**仍**报得出它」，即 `cleanup()` 制造的那个状态，也是**唯一**能分开两个账本的状态。
> 重测（每格附变异落地证明）：**D2／D3／C1／C2 四个全部 `rc=1 fail 14`**，正确代码 `rc=0 309/0`。
> ⚠️ **承重性也实测，且只有一步承重**：砍步骤 B ⇒ D3 复活；砍 A、砍 C ⇒ 变异仍死。
> **A 与 C 保留的诚实理由是「诊断」不是「检测」**，不再声称三步都是守卫。
>
> ⬇️ 以下 2026-09-06（第一节）及更早各节仍然有效。

---

## 🆕 2026-09-07（**仅登记文档，产品代码零改动**，仍为 v0.4.17）：preference 的「永不 supersede」把复述变成了累积

**用户报告注入上下文大量重复。已审计定位并登记为 [ADR 0014](decisions/0014-preference-exemption-turns-restatement-into-accumulation.md)。本轮只写文档，不改代码。**

**症状（实测，2026-09-07，8 repo 库 + global）**：按生产排序模拟 `5ed2b4d2` 的注入包，
**前 10 位有 9 条是同一条工作流规则的不同措辞**；双库独立复现（9/20、10/19）。
伤害不止冗余——**被同义句占满的位置，就是 `coding`/`procedure` 没能进包的位置**。

🆕 **次日复核（本轮亲测，与真实注入包逐条吻合 9/9 同序）：L3 画像回归后症状不缓解反加剧。**
global 的 L3 画像已重建（307 字符，09-06T17:09），personal 侧改走 derived 分支只占 1 条，
**省下的预算被 repo 侧最前列的同族 preference 原样吃掉**。
⛔ **判别式不是「占了几成」，是「挤掉了几条」**：该库可注入 **30 行**
（coding 14 / preference 12 / procedure 3 / fact 1），**整包只装下 8 行 repo 行，另 22 行未进模型**；
进包的是 **preference 7 条（全部同族）+ coding 1 条**，
**`procedure` 与 `fact` 本轮占比为 0——一个 kind 被整类清零**（计价 1221/1300 tok）。
⛔ **且 `INJECT_TOP_N=20` 在这条路上从未生效**：真正的约束是 token 预算（8 条即耗尽），
**「调大 TOP_N」对本症状零效果**——与下文证伪 `RECONCILE_EXISTING_LIMIT` 同型，
**两个看起来相关的参数，都不是正在起作用的那把尺子。**

⛔ **判别式不是「模型没认出重复」，而是「它的可选动作里没有正确答案」**：
实测该库有 **13 条 preference 处于 `superseded` 且 `superseded_by` 全为 `NULL`**
——`NULL` 即「被 drop，而非被替换」。**模型看见了重复、也处置了 13 次**，
但 §3.4 只给它 `drop`（丢新的、留旧的）和 `activate`（并列新增）两个选项，
**第三个选项 `supersede` 正是被 kind 豁免掉的那个**。于是形成棘轮：
**早期措辞永远沉淀，无法被更好的表述取代；只增不减，且增减都不改善质量。**

**决定性对照组**：`preference` **49 行 / `superseded_by` 命中 0**；
非 preference **539 行 / 命中 44**。**不是「很少」，是结构性的零**——
同一个 reconcile、同一批写者，差别只在 `reconcile.ts:222` 那句 `kind !== 'preference'`。

⛔ **本轮主动证伪了最显眼的那个修法，勿再提**：「`RECONCILE_EXISTING_LIMIT=30`
只覆盖 222 行的 13.5%，调大即可」——**窗口里本来就有 4 条同族旧条目**（实测），
模型看得见仍然新增。**窗口是加剧因素（覆盖率随库增长单调下降），不是成因；
只调这个参数，症状不会消失。**

**根因是三条各自正确的规则的乘积**（ADR 0010 教训 1 同型）：
豁免规则（不收敛）× `human` 优先级最高（排最前）× 固定预算（挤掉别人）。
**没有任何一方的测试会看到乘积**；更直接的是，现有测试**正在锁定**这个行为，
故修法落地时应预期它们翻红——按本仓已登记判据「区分钉住不变量与钉住当前行为」处置。

🟡 **`similar`/`replaces` 机制存在且正确，但够不着这条路径**（§871 早已预见「同一偏好存 3 次得 3 份」）：
它是**写后提示**（行已落库，收敛要等一个无人保证会发生的「下次」），
且**自动 extract 路径根本不经过 `propose`**。

**⛔ 修法方向已于同日被外部调研修正，见
[research-2026-09-07](research-2026-09-07-preference-convergence-prior-art.md)。**
原案「放宽豁免、允许覆写」**不可照做**：arXiv 2605.12978 实测**持续 consolidation
会把有用记忆改坏**（GPT-5.4 在自己先前无记忆就解出的 ARC-AGI 题上因记忆失败 **54%**，
效用可**跌破无记忆基线**），且论文点名的最坏输入形状——**近义重复流**——
**正是本缺陷的输入形状**。反向亦有实证：Mem0 v3 退回 ADD-only 后出现
[mem0#4956](https://github.com/mem0ai/mem0/issues/4956)（矛盾条目累积、
排序不含 recency ⇒ 旧事实被检索到），**与本仓症状同形**。
⭐ **两端的公共解本仓已经有了**：`superseded_by` + `procedure` 的版本化处置
**就是** bitemporal supersession（逻辑收敛、物理保留），
**preference 只是被 `reconcile.ts:222` 那行 `kind !== 'preference'` 排除在外**。
⚠️ **前置未决项**：`superseded` 属 `EXCLUDED_STATUSES`（`types.ts:165`），
**被排除出每一个读取面**——对「注入不再重复」充分，
**对「用户能否找回被收敛掉的偏好」不足**，须先决定是否给它一条可召回路径。
**落地顺序据此调整为 2 → 1**：先做**注入端按族去重**（零不可逆风险、直接消除实测症状、
被论文「显式 gate」与 Mem0 v3「读期收敛」双重支持），再动 reconcile。
**历史 49 条 preference（含 24 条同族）的合并是独立的第二件事，
会改写用户真实记忆库，须单独出方案并经用户确认**（沿用 ADR 0010「回收与代码修复可分开」的先例）。

---

## 🆕 2026-09-07（第二节，**未发版**，仍为 v0.4.17）：一条规则两个面，只守住了发版面

**本轮结项 2026-09-06（第一节）待办 1（四个 `lib/` 写入点：发版面已守，工作树面未守）。**
**只动 `test/` 六个文件，`src/` 与 `lib/` 零改动；交付物 tarball sha256 与已发布的 v0.4.17 完全相同。**

### 判据：不是「探针会不会被发出去」，是**测试往交付物目录里写**

`package.json` 的 `files` 是通配符 `lib/**/*.js`，**它不区分「编译产物」与「某个测试三秒前写进去的探针」**。
四个写入点（`guidance:50`、`layers:~3687`、`pipeline-e2e:~391/~457`）直接往 `lib/` 里写模块。
既有的 `package.test.mjs` 只守**发版面**（「不许被打进包」），**没有任何东西守写入面**（「不许写进 `lib/`」）。

**四个探针实测真的活在 `lib/` 里**（本 agent 1ms 轮询真实 `node --test`，非推理）：

```
tools.guardprobe-2876668.js
constants.extract-probe.88d0c890-….js
constants.guard-probe.bf59b707-….js
constants.ceiling-probe.2d9c75f8-….js        >>> 4/4 全部观测到 <<<
```

⛔ **发版面守卫是一个时间窗，两个方向都会错**：已登记的假红实证
（`+ [ 'lib/tools.guardprobe-4162710.js' ]`），以及**它也会漏掉真泄漏**——后者本轮由 F4 确定性证明。

### 修法：一个获取点 `withLibProbe`，把副本放在 `lib/` 的**同级**目录

```js
export const withLibProbe = async ({ file, mutate }, use) => { … }   // helpers.mjs
```

`mutate: null` 支持**未变异的控制副本**（`group.test.mjs` 的对照需要它，否则对照的就不是同一套机器）。
**六个站点全部收敛到它**：全测试树 `cpSync` 只剩 `helpers.mjs:798` 一处（本 agent `grep` 复核）。
成本实测可忽略：`lib/` 836K，`cp -r` 约 0.8ms × 7 次调用；全量 19.48/19.63/19.50s vs HEAD 19.43/19.41/19.59s。

### ⛔ 两条守卫，且实测**互相杀不掉对方的变异**——故两条都要，不是冗余

| 守卫 | 位置 | 杀得掉 | 杀不掉 |
|---|---|---|---|
| **写入处断言**（目的地不在 `lib/` 内） | `helpers.mjs` | 目的地被算进 `lib/`（含**计算出来的**路径） | 绕过 `dir` 另写一处 |
| **目的地感知的静态检查** | `package.test.mjs` | 源码里把 `lib/` 写成目的地（含别名链） | 计算/跨模块/运行时拼装的路径 |

**承重性双向实测（本 agent 独立复现，非引用）**：

```
写入处断言：忠实变异体在场 rc=1 305/5（开火 5 次） -> 删掉断言 310/310/0 全绿复活   => 承重
静态检查  ：真实瞬时泄漏在场 rc=1 309/1（点名 helpers.mjs:794） -> 删掉检查 309/309/0 全绿 => 承重
```

⛔ **「把探针目录移进 `lib/`」这个变异体证明不了断言承重，这是本轮最重要的方法论收获**：
删掉断言它**照样红**，因为 **Node 的 `cpSync` 自己就拒绝** `Cannot copy …/lib/ to a subdirectory of self`。
**执行 agent 自己发现了这一点并拒绝照抄 lead 给的说法**，改用绕开该 backstop 的忠实变异体
（在 `lib/` 外暂存 → `renameSync` 移入，目的地用 `['l','i','b'].join('')` 计算），才真正隔离出断言。
**判据照旧：变异体必须只改被测的那一个语义；一个「顺带被别人杀掉」的变异体是弱变异体。**

### ⛔ 三个候选守卫被实测否决（勿再提议，原文与读数一并登记）

| 候选 | 实测 | 结论 |
|---|---|---|
| `helpers.mjs` 里的 `after()` 钩子（照抄上一轮 tempRoot 的成功形状） | 采样竞态：trial1 **全漏**，trial2/3 各抓 1；且**点名 `auto-extract`／`lifecycle`／`command` 这些一个字节都没写的文件** | ⛔ 否决。**照抄上一轮的成功形状会退化成假消息**——探针寿命约 0.1s，钩子在本文件跑完时才看 |
| 朴素 grep 静态检查 | **对正确代码开火**（`group.test.mjs:1476` 的 `cpSync(libDir, dir)` 里 `libDir` 是**源**参数），**同时漏掉真实变异** | ⛔ 否决。必须解析**目的地参数位** |
| ESM 里 monkey-patch `node:fs` | `TypeError: Cannot assign to read only property 'writeFileSync' of object '[object Module]'`，16 文件红 15 | ⛔ 结构上不可能，ESM 命名空间对象冻结 |
| 只靠既有 `package.test.mjs`（发版面） | helper 改坏后 **3/3 全绿存活** | ⛔ 不充分，这是加写入面守卫的直接理由 |

### 🆕 代码审查打回（PASS WITH FIXES），三项必修均已修

审查独立复核了本节全部行为性论断、**一条也没能证伪**，但查出三项：
①**假注释**「(it validates `dir`, which that mutant never uses)」——**那个变异体确实用了 `dir`**，
断言沉默是因为 `dir` 仍然正确、泄漏是**另一处额外的写**；
②**假注释**「not fooled by `lib/../lib-probe-x`」——该路径是**被允许的同级目录**，不是对抗输入；
③**死代码** `rel !== ''`（本 agent 独立复现四种情形两谓词完全一致），**连同声称它在起作用的注释一并删除**。

### 🆕 QA 打回两个**真缺陷**，均已修（本 agent 独立复现，非引用）

| # | 缺陷 | 实测 |
|---|---|---|
| **F1** | `symlinkSync`／`linkSync` 在 `WRITERS` 里却**不在 `DEST_INDEX` 里**，`?? 0` 去读了**源**参数 | Node 语义本 agent 亲测：二者创建的都是**第 1 个参数**。**双向都错**：真泄漏 `findings: []` 漏报，而**对正确代码**（从 `lib/` 建链接）误报。⛔ **这正是该设计自称要避免的错误，出现在它自己的表里** |
| **F2** | `mkdtempSync` 不在 `WRITERS` 里 | planted 瞬时泄漏：**`310/310/0` 全绿，两条守卫全哑**。目的地是**可静态读出的字面量** `join(libDir, 'scratch-')`，**不属已披露的三类逃逸** |

⛔ **根因不是漏了两个名字，是 `WRITERS` 没有完备性压力**：执行 agent 实测发现比 QA 报的更糟——
**12 个条目里 9 个可被静默删除**（QA 报 6 个）。已改为 `WRITER_TABLE` 表驱动。
⚠️ **但表驱动本身不够，这是实测不是推理**：自检遍历该表，**删一行会连它自己的用例一起删掉**，
12/13 仍然全绿——**自指的循环钉不住自己的成员资格**。
真正起作用的是**独立的成员资格断言**（把 13 个 `name@dest` 写成字面量）。本 agent 复核：

```
删 symlinkSync／mkdtempSync／createWriteStream／openSync／copyFileSync  ->  5/5 全部 rc=1 pass1/fail1
把 symlinkSync 的 dest 由 1 翻回 0（即 F1 原缺陷本身）              ->  rc=1，红消息直指 symlinkSync@0 vs @1
F2 的 mkdtempSync 真泄漏                                            ->  rc=1，点名 helpers.mjs:804
```

### 🟡 残留缺口，明说不粉饰（全部已写进注释）

1. **自检本身无人守**（QA 的 C3，本仓历史上「两个无害变异组合成真盲区」的同型）：
   真实泄漏 ＋ 删自检 ＋ 掏空 `libAliases` ⇒ **全绿**。`libAliases` 只被自检杀死，而自检**没有守卫**。
   **判定为不可从内部关闭**（守自检的东西还需要它自己的自检），故**登记而非假装解决**。
2. **写者侧缺口**：`fs.promises.writeFile`、写者标识符别名（`const wf = writeFileSync`）、
   `execFileSync('cp', …)` **四种全部漏报**，且目的地可读。**今日为潜在**（本树纯同步：14 处 `node:fs`、0 处 promises）。
3. **M19**：把最终断言改成 `deepEqual(findings, findings)` 仍全绿——**等式两边同一表达式，恒真**，
   文件内任何断言都测不出来。已加 sentinel 关掉较弱的形态，**完整形态仍开放**，需外部变异测试。
4. **静态检查会扫到自己**，故本文件注释里出现字面量写入 `lib/` 的文本即成 finding（本轮撞到 4 次），
   已用「把字符串拆开拼装」处置并就地说明。
5. **计算/跨模块/运行时拼装的目的地**对静态检查不可见——**这正是写入处断言存在的理由**，二者互不包含。

### 硬约束复验（本轮实测，非引用）

| 约束 | 结果 |
|---|---|
| 全量基线 | ✅ `npm run verify` **rc=0**，**310/310/0**，独立 3 跑 |
| 测试只增不减 | ✅ 名称 diff vs HEAD：**+1、删 0、改名 0**，新增即那条静态检查 |
| 产品代码未动 | ✅ `git diff --name-only -- packages/memory/src packages/memory/package.json` **为空** |
| **交付物字节零变化** | ✅ tarball **59 条目**、探针匹配 **0**、sha256 **`4901c38b…`**，与已发布 v0.4.17 **完全相同** ⇒ **不发版** |
| 四个探针不再进 `lib/` | ✅ 1ms 轮询真实全量：`lib/` 内探针 **0** |
| 一条规则一个实现 | ✅ 全测试树 `cpSync` 真实拷贝只剩 `helpers.mjs:798`；六站点全走 `withLibProbe` |
| 清理与隔离 | ✅ `use()` 内断言抛出仍删副本；pid＋seq 跨进程不撞；临时目录泄漏 **0**、残留同级目录 **0** |
| 成本 | ✅ 19.48／19.63／19.50s vs HEAD 19.43／19.41／19.59s，在噪声内 |

### 本轮登记的待办

| # | 事项 | 现状（实测 2026-09-07） | 处置判据 |
|---|---|---|---|
| 1 | 🟡 **自检无人守（C3）** | 真实泄漏＋删自检＋掏空 `libAliases` ⇒ 全绿 | **登记而不修**。判据：这是自指问题，**不要为它再造一层自检**——那只是把同一个洞挪高一层。真正的关闭手段是**外部变异测试**（如本轮 QA 所做），应在每次动这个谓词时重跑 |
| 2 | 🟡 **写者侧缺口（async／别名／子进程）** | 四种全部漏报，目的地可读；今日不可达（本树纯同步） | **登记而不修**。**可机检的判据**：`grep -c "node:fs/promises" test/*.mjs` 若不再为 0，必须把 promises 写者加进 `WRITER_TABLE` |
| 3 | 🟡 **M19：断言自比较** | `deepEqual(findings, findings)` 全绿，文件内不可测 | **登记而不修**，已加 sentinel 关掉较弱形态 |
| 4 | 🟢 **静态检查对 temp fixture 的 `lib/` 会误报** | 今日无人这么写；`package.test.mjs` 已在 stage 安装，只因未命名 `lib/` 才沉默 | **登记而不修**。判据：一旦有测试要在临时夹具里造 `lib/`，此处需加豁免或改名 |
| 5 | 🆕 ⛔ **方法论：照抄上一轮的成功形状会退化** | `after()` 钩子在 tempRoot 那轮是正解，本轮却因资源寿命由「整跑」变成「0.1s」而退化为漏报＋点名无辜文件 | **判据（下次照做）**：换问题时**不要问「上次用的什么」，要问「这个守卫杀得掉哪个别的候选杀不掉的变异」**，并把每个候选真的实现出来跑一遍。本轮四个候选实测死三个 |

---

## 🆕 2026-09-06（第二节，**未发版**，仍为 v0.4.17）：丢掉的不是清理，是句柄

**本轮结项 2026-09-06 待办 3 ／ 2026-09-05 待办 3（`makeRepo()` 的外层 `tempRoot()` 从不清理）。**

### 判据：不是「谁忘了清理」，是**外层句柄被丢弃，因而无法清理**

`mkdtempSync` 返回它刚造出来的目录的**唯一**句柄。HEAD 的写法把这个返回值直接喂给别人：

```js
export const tempRoot = () => mkdtempSync(join(tmpdir(), 'strataloom-test-'))   // helpers.mjs:15

const dir = join(tempRoot(), 'repo')     // 只留内层，外层名字当场丢失
cwd: tempRoot()                          // 值透传，外层名字当场丢失
```

**反证「清理与否」不是判别式**（这是本轮最先做的实测）：

```
store.test.mjs   tempRoot() 调用 20 次   ->  delta = 0     （写 const root = tempRoot(); ... cleanup(root)）
group.test.mjs   tempRoot() 调用  4 次   ->  delta = 102   （句柄丢弃）
```

**调用次数与泄漏量无关，句柄留没留住才相关。**

### 全量与逐文件基线（本 agent 独立复现，非引用）

取数一律用**排序快照 ＋ `comm -13` 差集**，不信裸计数（并发的别的东西会污染）：

```
ls -U /tmp | grep '^strataloom-test-' | sort > before
npm test
ls -U /tmp | grep '^strataloom-test-' | sort > after
comm -13 before after | wc -l
```

```
HEAD 全量 3 独立跑：  delta=213  delta=213  delta=213      三跑均 ℹ tests 309 / pass 309 / fail 0
```

逐文件（`node --test <file>`，同法取数）：

| 文件 | HEAD delta | 修复后 delta |
|---|---|---|
| `group.test.mjs` | **102** | 0 |
| `layers.test.mjs` | **83** | 0 |
| `service.test.mjs` | **11** | 0 |
| `e2e.test.mjs` | **7** | 0 |
| `inject.test.mjs` | **6** | 0 |
| `command.test.mjs` | **2** | 0 |
| `auto-extract.test.mjs` | **2** | 0 |
| 其余 9 个文件 | 0 | 0 |
| **合计** | **213** | **0** |

体量：实测 639 个泄漏目录共 **27,717** 个 inode，**均值 43.4 inode/目录** ⇒ 约 **9,270 inode/全量跑**。
`/tmp` 与 `/` 是**同一个文件系统**（`stat -f` fsid 同为 `65dbc4ab883aff0c`，`df` 同为 `/dev/nvme0n1p2`），
故它吃的是**整机**的 inode 预算，且**无上界**。

### ⛔ 证伪并改正本页「每跑 delta=9」（2026-09-05 一节的硬约束表与待办 3；原文保留，勿改）

| 旧登记 | 实测 |
|---|---|
| 「temp-dir **每跑** `delta=9`」 | ⛔ **范围标注缺失**。9 是 **e2e(7)＋command(2)＋lifecycle(0)** 的合计，即 v0.4.17 那轮的三文件范围 |
| 「每跑」＝全量？ | ⛔ 全量实测 **213**。本 agent 单跑那三个文件验证旧读数：`node --test test/command.test.mjs test/e2e.test.mjs test/lifecycle.test.mjs` ⇒ **delta ＝ 9，`tests 12 / pass 12 / fail 0`** |
| 定性 | ✅ **它当时是一次正确的测量，只是量的是那一轮的范围**——**不是 23 倍的测量错误，不要那样记**。缺的是「（本轮三文件范围内）」这几个字 |

**这才是它被 🟢「登记而不修」三轮的原因**：`delta=9` 读起来像是全量的每跑代价，
9 个目录看着无害；标上范围它就会显出是 213。
**教训：一个不带范围标注的资源读数，会把它自己的严重性写小一个量级。**

### 修法：只动 `test/helpers.mjs`，调用点零改动

在**唯一获取点**登记，由文件级 `after()` 兜底删除：

```js
const live = new Set(); const created = new Set()
export const tempRoot = () => { const dir = mkdtempSync(...); live.add(dir); created.add(dir); return dir }
export const cleanup = (dir) => { try { rmSync(dir, {recursive:true,force:true}) } catch {}; live.delete(dir) }
after(() => { for (const dir of live) { try { rmSync(dir, {recursive:true,force:true}) } catch {} } live.clear() })
```

**它满足本仓「在获取处注册收尾」那条规矩**：`tempRoot()` **就是**获取处，
在这里登记等于在获取处登记，且把 15 个执行点收敛成 **1 个**。
**它只 ADD 一层兜底，不 MOVE 任何既有 `cleanup(root)`**——每一处既有清理仍在原地照跑。

**删除判据是身份，不是模式**：只删本进程内 `mkdtempSync` 刚返回的值。
**绝不扫 `/tmp`、绝不 glob、绝不前缀匹配**——那会删掉**并发跑**的另一个进程的活目录。
`node --test` 每文件一进程，故这个 Set 天然隔离。

### ⛔ 为什么是 `after()` 而不是 `process.on('exit')`——**且二者互有胜负，`after()` 不是超集**

⚠️ **本节初稿是「说了半句真话」的形状，QA 已证伪，此处按实测改写（原读数不作废，是范围不全）。**
初稿写「`exit` 处理器确实覆盖断言失败、未捕获异常、显式 `process.exit()`」——
**这句关于 `exit` 的陈述是真的**，但它出现在「为什么不用 `exit`」的标题下，
读者只会得出「`after()` 至少不比 `exit` 差」。**实测为假。**

完整失败模式矩阵（两个**仅清理机制不同**的 helper，每格 3 次独立跑、私有 `TMPDIR`，读数为 `rc/leak`）：

| 失败模式 | `after()` | `process.on('exit')` |
|---|---|---|
| 断言失败 | **1/0** | 1/0 |
| **RC=124 挂起（活 30s interval）** | **124/0** ✅ | **124/1** ❌ |
| 未捕获 async rejection | **1/0** | 1/0 |
| **`process.exit()` 中途退出** | **1/1** ❌ | **1/0** ✅ |
| SIGTERM 中途 | 1/1 | 1/1 |
| SIGKILL 中途 | 137/1 | 137/1 |

**二者互有胜负**：`after()` 赢在 RC=124（它在该文件 test 跑完时就执行了，早于进程被卡死）；
`exit` 赢在 `process.exit()`（进程不经过「测试跑完」就拆掉，`after()` 根本没机会运行）。

**仍然选 `after()`，理由是本仓的性质而不是普适优劣**：
RC=124 挂死是本页**有文档、真实发生过**的形态；而 `process.exit()` **在本包里根本不存在**——
本 agent 自己 grep 确认，全包唯一命中就是这段注释本身：

```
grep -rn 'process\.exit' packages/memory/{src,lib,test}
  -> test/helpers.mjs:（该注释）   其余 src/ lib/ test/ 全部 0 命中
```

**判据**：若将来有人把 `process.exit()` 引入测试或 `src/`，**这个取舍即反转**，注释与本节必须重看。

### 🟡 残留缺口，明说不粉饰：**三种模式**仍各泄漏 1（不只 SIGKILL）

```
SIGKILL 中途（rc=137）        leak=1   不可约，两种机制都挡不住
SIGTERM 中途（rc=1）          leak=1   两种机制都泄漏；node --test 在信号路径上不排空文件级 after
process.exit() 中途（rc=1）   leak=1   after() 泄漏而 exit 不会；仅因本包无此调用才接受
```

⚠️ **初稿只写了 SIGKILL 一种，是清单不全**（QA 指出，已补齐）。
本轮关掉的是确定性的每跑 213 与 RC=124 挂死那条路径，**不是「全覆盖」，也不是 `exit` 的超集**。

### 🆕 回归守卫：第二个 `after()` 钩子，且问磁盘不问 Set

⛔ **plan 审查提议把守卫写成一条名为 `'zz: this file left no temp root behind'` 的 `test()`——那是错的**，
本轮实测：文件级 `after()`（含 helper 的清扫）在**所有 test 之后**运行，
写成 `test()` 只能读到**清扫之前**的状态，**会在已修好的代码上翻红**。
跨模块 `after` 为 **FIFO**（双模块探针实测：`HELPER SWEEP, size=1` → `GUARD after, size=0`，`pass 1 fail 0`），
故守卫写成**第二个 `after()`** 即可读到清扫之后的状态。

⛔ **守卫断言 `live.size === 0` 会被骗**（这是本轮实测出来、brief 里没有的一条）：

| 清扫实现 | `live.size` 守卫 | 磁盘 `existsSync` 守卫 |
|---|---|---|
| 正确清扫 | pass 1 / fail 0 | pass 1 / fail 0 |
| **只 `live.clear()` 不删文件** | **pass 1 / fail 0（被骗）** | **pass 1 / fail 1（抓到）** |
| 完全没有清扫 | pass 1 / fail 1 | pass 1 / fail 1 |

故守卫对**曾发出过的每一个 root** 做 `existsSync`。这也是 `cleanup()` 只从 `live` 里删、
**保留在 `created` 里**的原因：`cleanup()` 吞掉自己的失败，「调用过 cleanup」不是「目录已消失」的证据。

**守卫看着红过（红/绿双向实测）**：

```
清扫停用（全量）   rc=1   ℹ tests 316  ℹ pass 309  ℹ fail 7     delta=213
  auto-extract.test.mjs leaked 2 temp root(s) ...
  command.test.mjs      leaked 2 ...
  e2e.test.mjs          leaked 7 ...
  group.test.mjs        leaked 102 ...
  inject.test.mjs       leaked 6 ...
  layers.test.mjs       leaked 83 ...
  service.test.mjs      leaked 11 ...          >>> 2+2+7+102+6+83+11 = 213 <<<
清扫恢复（全量）   rc=0   ℹ tests 309  ℹ pass 309  ℹ fail 0     delta=0
```

**309 条通过的测试在红版里一条不少**——多出来的 7 条只是失败的钩子。

⚠️ **第一版守卫消息有两个缺陷，是看着红才发现的，已改**：
① `node --test` 把 helper 里注册的钩子失败**归属到注册它的文件**，
七条失败全部打印 `✖ .../test/helpers.mjs`，**没有一条点名真正泄漏的文件**——
现用 `basename(process.argv[1])` 点名被测文件（上面那七行就是改后的）。
② `group` 那条把 **102 个绝对路径**灌进 diff，把唯一有用的数字埋了——现只抽样前 5 个再报总数。

### 🆕 守卫为什么登记在 helper 里，而**不是**每个测试文件写一行

本仓已把「一条规则，N 个执行点，只守住一部分」登记为签名缺陷
（2026-09-05 待办 2：「`e2e` 探针缺 platform 维度，三文件三种实现」）。
在**拥有获取点的模块**里登记，使守卫**由构造保证不可遗漏**：
文件要泄漏一个 `strataloom-test-` root 就必须调 `tempRoot()`，
调它就必须 import 本模块，import 本模块就注册了这两个钩子。**没有需要同步维护的清单。**

⛔ **而且「每文件一行」的方案已经是不完整的，这是实测不是预判**：
`tempRoot()` 不是测试文件唯一的获取路径——`openRegistry()` 内部调它，
而 **`jobs`／`pipeline`／`pipeline-e2e` 三个文件用 `openRegistry` 且从不出现 `tempRoot` 这个词**。
brief 提议的完备性判据 `grep -l 'tempRoot' test/*.test.mjs` **列不出它们**，
即它会认证一个**漏掉真实获取路径**的文件集合。直接探测（只调 `openRegistry()` 的临时测试，停用清扫）：

```
zz-probe.test.mjs 中 grep -c tempRoot = 0        >>> 对 tempRoot 判据完全隐形 <<<
清扫停用 -> ℹ pass 1  ℹ fail 1
           zz-probe.test.mjs leaked 1 temp root(s) under /tmp: /tmp/strataloom-test-i5nL7P.
清扫启用 -> ℹ pass 1  ℹ fail 0   delta=0
```

**本方案的完备性判据**（可机检）：

```
grep -l 'strataloom-test-' test/*.test.mjs      # 必须无输出（实测：0 个文件）
grep -lE 'tempRoot|openRegistry' test/*.test.mjs | xargs grep -L "from './helpers.mjs'"   # 必须无输出（实测：0）
```

⛔ **brief 给的判据「`grep -c mkdtempSync test/*.mjs` 必须恰为 1」对本树不成立，本轮拒绝写入**：
实测 `helpers.mjs:4`（1 处 import ＋ 1 处调用 ＋ 2 处注释）、**`package.test.mjs:2`**。
`package.test.mjs` 直接调 `mkdtempSync` 造 `strataloom-pack-` 暂存目录，
**它留住了句柄、自己 `rmSync` 掉、实测泄漏 0**——**它是一个正确的调用点，不是漏改的**。

### 🆕 存量回收（与代码修复可分开的一件事）

修复前 `/tmp` 存量 **33.2 万** 个 `strataloom-test-*`，**已全部回收**：

```
回收前  IUse% = 99%   IFree ≈ 245,014
回收后  IUse% = 10%   IFree = 14,096,281        （/dev/nvme0n1p2，15,564,800 总数）
```

⛔ **不要再写「还剩约 26 次全量跑就耗尽 inode」**：那个读数**取时为真**，
但存量已回收，**现在写它就是对当下的一句假话**（按现值反而是约 1,500 跑）。
**该用来说明严重性的、稳定且仍为真的量是：每跑 213 个目录／约 9,270 inode，无上界，
且 `/tmp` 与 `/` 同一文件系统。**

### 🆕 代码审查打回（PASS WITH FIXES）：两个变异全绿存活 ＋ 一处假注释

审查独立复现了本节十四条行为性论断、无一被证伪，但查出 3 个必修项。**三项均已修**。

#### D1 🔴 假注释（本仓与代码缺陷同级）

`helpers.mjs` 原写 `(the other 10 test files: 0)`，**真值是 9**：
`ls test/*.test.mjs | wc -l` ＝ **16**，泄漏文件 7 个 ⇒ 16 − 7 ＝ **9**
（零泄漏者：`guidance jobs lifecycle package pipeline-e2e pipeline repo-key resilience store`）。
⚠️ **同一事实在 `docs/STATUS.md` 的逐文件表里写的是「其余 9 个文件」——那里对，helpers 里错。**
**一个事实两处书写、其中一处错**，正是本页反复登记的形状。已改 `10` → `9`，并把 9 个文件名写进注释，
使它不再是一个**需要读者自己重算才能证伪**的数字。

#### D2／D3 🔴 两个变异全绿存活：账本与判据无人守

本 agent 用**私有 `TMPDIR`** 独立复现（不是引用审查）：

```
D2  删掉 tempRoot() 里的 created.add(dir)      RC=0  ℹ tests 309  pass 309  fail 0  delta=0   存活
D3  守卫判据改读 [...live] 而不问磁盘           RC=0  ℹ tests 309  pass 309  fail 0  delta=0   存活
```

**两者都不破坏清扫**，故套件不会红；它们只**摧毁探测器**。
一个失效表现为「沉默」的守卫，必须有自己的阴性对照。

#### ⛔ 审查给的候选断言：**实测两个变异一个也杀不掉**，已拒绝采纳

审查明说该候选未经验证并要求我自测。实测结果是它**对任何状态都不开火，包括正确代码**：

```js
if (created.size > 0 && [...created].every((d) => live.has(d))) assert.fail(...)
```
```
候选 ＋ 正确 helper    RC=0  pass 309  fail 0     （从不开火）
候选 ＋ D2            RC=0  pass 309  fail 0     存活
候选 ＋ D3            RC=0  pass 309  fail 0     存活
```

**成因是结构性的**：钩子 FIFO，清扫已先跑过 `live.clear()`，故此处 `live` **恒为空**——
`created` 非空时 `every(d => live.has(d))` 恒 false，`created` 为空时 `created.size > 0` 短路。
**该谓词两个方向都不可满足，是一条死断言。** 按本页规矩，原文连同证伪读数一并登记，勿再提议。

#### 修法：一条**端到端阴性对照**，一次钉死 D2 与 D3

两个变异破坏的是**同一件事：守卫再也看不见一个真实存在于磁盘上的 root**。
故对照直接断言**这个能力本身**，而不是断言账本的形状：

```
1. 造一个 tempRoot() 形状的真目录  2. 经同一个获取点登记
3. 断言守卫自己的谓词 leakedRoots() 报出它
4. 删掉它，断言谓词随即安静
```

### ⛔ 复审第三次打回：上表**是假的**，D3 与 C2 并未被杀（原文保留在下，勿改）

**被证伪的原文（本轮第四次「为真实断言编造错误理由」，前三次由审查抓到，第四次由复审抓到）**：

> | 变异 | 被哪一步杀 | 消息 | 读数 |
> |---|---|---|---|
> | D2（无 `created.add`） | **步骤 3** | `...guard has gone deaf...` | `RC=1  tests 323  pass 309  fail 14` |
> | D3（谓词读 `live`） | **步骤 4** | `...reports a directory that is no longer on disk...` | `RC=1  tests 323  pass 309  fail 14` |
>
> D3 **能通过步骤 3**……删除后探针仍在 `live` 里，**步骤 4 才是唯一抓得住它的那一步**。

⛔ **根因是我施加了一个比复审更弱的变异体，并从弱变异体外推**。二者都是真读数，但**不是同一个变异**：

```
我的 D3：     const leakedRoots = () => [...live]                            -> rc=1  fail 14   （死）
复审的 D3：   const leakedRoots = () => [...live].filter(d => existsSync(d))  -> rc=0  309/0     （存活）
```

**复审的形态才是现实的那个**（它保留 `existsSync` 过滤），而**旧的步骤 4 在数学上不可能区分二者**：
它跑在 `rmSync(probe)` 之后，`existsSync(probe)` 为 false，**两个谓词都把探针过滤掉、都通过**。隔离验证：

```
旧步骤3   good=true  bad=true      <- 都通过，无区分力
旧步骤4   good=true  bad=true      <- 都通过，无区分力
新步骤B   good=true  bad=FALSE     <- 唯一能分开的状态
```

**判别式**：读 `live` 与读 `created` **唯一可观测的差异**，是「**目录还在磁盘上、但已被移出 `live`**」——
正是 `cleanup()` 制造的状态，也正是坏清扫留下的状态。**旧控制从未构造过这个状态。**

### 🆕 修法：用 `cleanup()` 的那个状态做步骤 B（复审给的形状，本 agent 实测后采用）

```
A. 经 tempRoot() 造真目录，断言谓词报出它
B. 从 live 移除但【不删磁盘】，断言谓词【仍】报出它     <-- 唯一杀得掉 D3 的一步
C. 真删掉，断言谓词随即安静
```

**变异落地证明 ＋ 读数（每格独立私有 TMPDIR）**：

```
正确代码                                                     rc=0  309/0     leak 0
D2  created.add 行数 0                                       rc=1  fail 14   leak 0    "gone deaf"
D3  pred=[...live].filter((dir) => existsSync(dir))          rc=1  fail 14   leak 0    "wrong ledger"
C1  created.add 行数 0 ＋ 清扫 rmSync 块 0                    rc=1  fail 14   leak 213  "gone deaf"
C2  清扫 rmSync 块 0 ＋ pred=[...live].filter(existsSync)     rc=1  fail 14   leak 213  "wrong ledger"
```

⚠️ **承重性也实测了，且结论是「只有一步承重」，不许再说三步都承重**：

```
砍掉步骤 B ＋ D3                       rc=0  309/0    存活   => B 承重
砍掉步骤 A ＋ D2                       rc=1  fail 14  仍死   => A 不是唯一捕获点（B 也要求报出探针）
砍掉步骤 C ＋「从不过滤」的谓词         rc=1  fail 14  仍死   => C 无专属变异
                                                            （它被【主守卫】抓到，报 "leaked 2 temp root(s)"）
```

**A 与 C 仍然保留，诚实理由是「诊断」而非「检测」**：它们把一次「探测器坏了」拆成三条不同消息，
省去二分定位；成本是一次 mkdir 一次 rmdir。**若将来强制「一断言一变异」，不可删的是步骤 B。**

14 条失败 ＝ import 本模块的测试文件数
（`grep -l "from './helpers.mjs'" test/*.test.mjs` ＝ 14），即探测器**一次性全线失效**，符合预期爆炸半径。

### 🆕 QA 打回 D7：**组合变异全绿却泄漏 213**——同一条自检已覆盖（实测，非推测）

QA 指出单个看无害的变异**两两组合即成真盲区**：套件 `309/309/0` 全绿，`/tmp` 却泄漏全部 213 个目录。
这正是本节守卫注释自己论证过的失效面（「坏清扫 ＋ 问 Set 的守卫 ⇒ 被骗」）——**论证对了，但当时没有执行点守住它**。

QA 要求我加完自检后**直接把 C1／C2 跑一遍并如实报告**，不许声称覆盖。

⚠️ **第一次报告的 C2 读数是错的**（与 D3 同因：变异体施加在较弱的形态上），
复审证伪后**已用复审的 C2 形态重测**。**修法改成步骤 B 之后**的读数：

| 组合变异 | 加自检前 | 步骤 B 之前的旧控制 | **步骤 B 之后（现状）** | 死于 |
|---|---|---|---|---|
| **C1** ＝ 去 `created.add` ＋ 清扫提前 `return` | `RC=0  309/0`，**leak 213**（静默） | 已转红 | **`rc=1  tests 323  pass 309  fail 14`**，leak 213 | 步骤 A（`gone deaf`） |
| **C2** ＝ 清扫只 `live.clear()` ＋ 谓词 `[...live].filter(existsSync)` | `RC=0  309/0`，**leak 213**（静默） | ⛔ **仍存活（`rc=0 309/0`）** | **`rc=1  tests 323  pass 309  fail 14`**，leak 213 | 步骤 B（`wrong ledger`） |

⚠️ **措辞必须准确**：变异体下 leak **仍是 213**，因为那里的清扫是真坏了；
**改变的是套件不再对此撒谎**（从 `309/309/0` 变成 `rc=1`）。
C1 与 D2 同因、C2 与 D3 同因——**故未为组合另造第三套机器**，符合 QA 的要求。

### 🆕 QA 复核为「仅登记、无害」的存活变异（本 agent 独立复现）

```
M4  去掉 cleanup() 里的 live.delete(dir)      RC=0  309/0  leak=0    存活且无害
M8  GUARD_SAMPLE = 0                          RC=0  309/0  leak=0    存活且无害
```

M8 的消息退化实测（配坏清扫制造真泄漏）：

```
command.test.mjs leaked 2 temp root(s) under /tmp/tmp.nebJ6mxNRH: , ... (+2 more). Each was ...
```

**文件名与计数仍可行动**，只是样例列表空掉，与 QA 的 C3 读数一致。**故不修，仅登记。**

#### 🆕 并发全量下 `package.test.mjs` 假红：一次**真实命中**（此前只是预测）

审查并发跑两个全量套件时实测到：

```
AssertionError: the tarball carries files under lib/ that no source compiles to
+ [ 'lib/tools.guardprobe-4162710.js' ]
```

这是 `guidance.test.mjs:50` 把探针写进 `lib/` 与并发 pack 撞车，
即**本页 2026-09-06（第一节）待办 1 已登记的「假红」方向的第一次实证观测**——
那条判据当时写的是「**既可能**漏掉泄漏，**也可能**把并发写入的探针打进包里变成假红」，
**假红那一半此前只是预测，现已被观测到**。
**与本轮改动无关**：本轮 diff 不含 `guidance.test.mjs`，单跑与串行全量均不复现。
⚠️ **两个读数不矛盾，且必须一起记**：QA 在隔离环境 **0/11 未复现**，审查在共享 `/tmp` 并发时**命中 1 次**。
**它是时间窗，不是「有或没有」**——本页第一节待办 1 已把它定性为时间窗，此处是那条定性的实证。

### 🆕 QA 复核通过的几项（本 agent 引用 QA 读数，已注明来源）

以下为 **QA 的读数**，本 agent 未逐项独立复现，故明确标注来源而非冒充自测：

| 项目 | QA 读数 |
|---|---|
| **成本为零** | HEAD 均值 **19.52s** vs 修复版 **19.45s**，差 **−0.07s**，在噪声内；`layers` 泄漏 83→0 而仅 +0.09s |
| **并发安全** | 两套件共享同一 `TMPDIR` 并发跑，双双 `309/309`；**外部长驻持有者存活未被误删** |
| **身份删除判据成立** | 人造 50 个同前缀假目录，**50/50 存活、0 损坏**——证实「只删本进程 `mkdtempSync` 的返回值」 |
| **稳定性** | 7 次连跑 ＋ `--test-concurrency=1` ＋ 3 次随机顺序，**全部 309/0、leak 0** |
| **测试名哈希** | 与 HEAD 完全相同 `ebcf7e86…` |

⚠️ **关于测试名哈希的一处澄清（不是分歧）**：本 agent 自己的归一化管线算出的是
`45d1e185…`，**但 HEAD 与修复版在同一管线下完全相等**，`diff` 输出 0 行。
**绝对哈希取决于归一化方式**（是否含 `✔` 前缀、是否剥耗时、排序方式），
故 `ebcf7e86…` 与 `45d1e185…` 是两条管线，不是两个结论。**可核验的断言是「二者相等」，不是某个具体哈希。**

### 硬约束复验（本轮实测，非引用）

| 约束 | 结果 |
|---|---|
| 全量基线 | ✅ **309/309/0**，修复后独立 3 跑 ＋ `npm run verify` 第 4 跑，均 rc=0；审查打回修复后 `npm run verify` 再跑 **rc=0、309/309/0、`cancelled 0`** |
| 测试名逐字符相同 | ✅ HEAD 309 行 ↔ 修复版 309 行，`diff` **输出 0 行** |
| 测试数未变 | ✅ 309 → 309。**钩子不是测试**：通过的钩子不计数，失败的钩子才多报（停用清扫的红版 316 ＝ 309 ＋ 7；D2／D3 红版 323 ＝ 309 ＋ 14） |
| 产品代码未动 | ✅ `git diff --name-only -- packages/memory/src` **为空** |
| 改动范围 | ✅ `git status --short` 仅 `M packages/memory/test/helpers.mjs`、`M docs/STATUS.md`；调用点**零改动** |
| 泄漏 213 → 0 | ✅ 全量 3/3 `delta=0`；逐文件 **16/16** `delta=0`（快照 `comm -13` 取数） |
| 守卫看着红过 | ✅ 停用清扫 ⇒ `rc=1`、`fail 7`、七条各自点名文件，计数合计 213；恢复后 `fail 0` |
| **阴性对照看着红过** | ✅ 步骤 B 定稿后重测：D2 ⇒ `rc=1 fail 14`（A，`gone deaf`）；**D3 ⇒ `rc=1 fail 14`（B，`wrong ledger`）**；正确代码 ⇒ `rc=0 fail 0`。**每格均附变异落地证明**（`grep -n 'const leakedRoots'` ／ `created.add` 行数 ／ 清扫块计数） |
| **变异不再存活** | ✅ **D2／D3／C1／C2 四个全部 `rc=1  tests 323  pass 309  fail 14`**（C1／C2 同时 leak 213，因其清扫真坏）。⚠️ 其中 **D3／C2 在上一版控制下是存活的**，见本节复审打回一段 |
| **承重性（诚实版）** | ✅ 砍步骤 B ＋ D3 ⇒ `rc=0 309/0` **复活** ⇒ B 承重；砍 A ＋ D2 ⇒ 仍 `rc=1`；砍 C ＋「从不过滤」谓词 ⇒ 仍 `rc=1`（被**主守卫**抓到）。**故只有 B 承重，A／C 保留理由是诊断而非检测** |
| **失败模式矩阵** | ✅ 6 种模式 × 2 种机制 × 3 次 ＝ 36 跑，全部可复现；**`after()` 与 `exit` 互有胜负**，已按实测改写注释 |
| **`process.exit()` 在本包不存在** | ✅ `grep -rn 'process\.exit' packages/memory/{src,lib,test}` 唯一命中是该注释自身 |
| 工作树可构建 | ✅ `npm run verify`（`tsc && node --test`）**rc=0** |
| 本 agent 未留残渣 | ✅ 全程用**私有 `TMPDIR`**（`export TMPDIR=$(mktemp -d)`，实测对 `os.tmpdir()` 生效）跑实验，用完即删；收尾共享 `/tmp` 读数 ＝ **0** |

⚠️ **取数陷阱不变，且本轮全程遵守**：必须 `ls -U /tmp | grep -c '^strataloom-test-'`；
`ls -d /tmp/strataloom-test-*` 会因 argv 溢出**静默返回 0**。
本轮另加两条：**裸计数不够，要用排序快照 ＋ `comm -13` 差集**，
否则并发跑的别的东西会把读数污染成任意值。
**更彻底的办法是私有 `TMPDIR`**：`export TMPDIR=$(mktemp -d)` 实测对 `os.tmpdir()` 生效
（`TMPDIR=$T node -e "console.log(require('os').tmpdir())"` 打印 `$T`），
此时 `ls -U "$TMPDIR" | grep -c '^strataloom-test-'` 直接就是 delta，**不受并发 agent 干扰**。
审查期间确有并发 agent 在动共享 `/tmp`，本轮打回后的全部实验均在私有 `TMPDIR` 内完成。

### 本轮登记的待办

| # | 事项 | 现状（实测 2026-09-06 第二节） | 处置判据 |
|---|---|---|---|
| 1 | 🟡 **SIGKILL 途中仍泄漏** | 实测：测试执行途中 `kill -9` ⇒ 泄漏 1。进程内方案的不可约下界 | **登记而不修**。真正的修法只有进程外的（如 systemd-tmpfiles、CI 的 job 级 `/tmp` 隔离）。**不要试图用 `process.on('SIGKILL')` 之类去修——SIGKILL 不可捕获** |
| 2 | 🟢 **`package.test.mjs` 的 `strataloom-pack-` 暂存目录不在本机制覆盖内** | 它自己留句柄自己删，**实测泄漏 0**。但它的清理写在 `try/finally` 之外的两处 `rmSync`，与本文件的兜底机制无关联 | **登记而不修**（当前无泄漏，改它是无因之改）。判据：下次动那个文件时，让它也走 `helpers.mjs` 的登记机制，则清扫与守卫自动覆盖 |
| 3 | 🟢 **D4／M4：`cleanup()` 里的 `live.delete(dir)` 是存活变异** | 审查与 QA 均实测存活；**本 agent 独立复现：`RC=0  309/0  leak=0`**。**但它无害**——`rmSync(force:true)` 对已消失路径是 no-op，删掉只让清扫多做一次空操作。**且注释从未对它作过承诺，故不是假注释**（这条区分是审查明确给出的，照录） | **登记而不修**。属整洁而非正确性。判据：不要为它编一条断言——**为一个无害差异写断言，等于把「守卫」这个词贬值** |
| 3b | 🟢 **M8：`GUARD_SAMPLE = 0` 存活但无害** | 本 agent 独立复现：`RC=0  309/0  leak=0`。配坏清扫制造真泄漏后消息退化为 `... : , ... (+2 more).`——**样例列表空掉，但文件名与计数仍可行动**，与 QA 的 C3 读数一致 | **登记而不修**。判据：若将来消息里连计数也可被变异掉，再补断言 |
| 4 | 🟢 **D5：`survivingRoots` 无消费者** | 注释自称 "Exported for diagnosis"，**与事实相符、不构成假注释**（审查判定，本 agent 同意）。已改为复用 `leakedRoots()`，与守卫共用同一谓词 | **保留，无需改动**。判据：若将来它长期无人调用且注释开始声称别的用途，再一并删 |
| 5 | 🟡 **并发全量下 `package.test.mjs` 假红：已从预测变为观测** | 审查并发跑两个全量时命中 `+ [ 'lib/tools.guardprobe-4162710.js' ]`，即 `guidance.test.mjs:50` 的探针与并发 pack 撞车；**QA 在隔离环境 0/11 未复现**——两读数不矛盾，它是时间窗 | **归入 2026-09-06（第一节）待办 1**，不另开条目。**它是那条判据「假红」一半的首次实证**，与本轮 diff 无关。判据不变：把那四个写入点改到 `lib/` **之外** |
| 6 | 🟡 **`process.exit()` 一旦被引入，`after()` 的取舍即反转** | 实测 `after()` 在 `process.exit()` 中途退出时**泄漏 1**，而 `process.on('exit')` 泄漏 0。当前可接受**仅因本包无此调用**（`grep -rn 'process\.exit' packages/memory/{src,lib,test}` 唯一命中是注释自身） | **登记而不修**。**判据是可机检的**：上面那条 grep 若出现第二个命中，必须重看 `helpers.mjs` 的清理机制与其注释。**不要在此之前改成 `exit`——那会丢掉 RC=124 挂死这条真实发生过的形态** |
| 7 | 🟡 **SIGTERM 中途仍泄漏 1（两种机制都挡不住）** | 实测 `rc=1  leak=1`，`after()` 与 `exit` 表现相同；`node --test` 在信号路径上不排空文件级 `after` 钩子 | **登记而不修**，与待办 1 的 SIGKILL 同类。真正的修法在进程外（`systemd-tmpfiles`／CI 的 job 级 `/tmp` 隔离） |
| 8 | 🆕 ⛔ **方法论教训：变异体太弱 ＝ 假绿的镜像，会产出假注释** | 本轮第四次同型缺陷即由此而来：我把谓词改成裸 `[...live]`（**丢掉 `existsSync` 过滤**）就宣布「杀掉了」，而**保留过滤的现实形态完整存活**。弱变异体让我误信控制有效，并据此写下一句数学上不可能为真的注释 | **判据（下次照做）**：①**变异体必须保留被测代码的其余结构**，只改要测的那一个语义——丢掉一个过滤器就是换了另一个更容易死的对象；②**每条变异读数必须附落地证明**（贴改后那行原文 ＋ 计数），本轮起已强制；③**「砍掉这条断言看是否复活」是判断承重性的唯一方法**，不许用推理代替——本轮正是它揭穿了「三步都承重」 |

---

> 最后更新：2026-09-06（第一节） · **本轮不发版，版本仍为 v0.4.17**（判据见本节末）：
...
> **⛔ 2026-09-06（第二节）结项本待办 3（原文保留在上，勿改）**：
> 这一行是对的（存量确实在长），**但它继承了上一轮「每跑 delta=9」的无范围读数**，
> 因而连续第三轮被判为 🟢 无害。**全量每跑实测 213，不是 9。**
> 已修（`test/helpers.mjs` 单文件，调用点零改动，实测 213 → 0），存量 33.2 万个已回收。
...
<!-- ⛔ 2026-09-06（第二节）证伪：上一行的「每跑 delta=9」缺范围标注。9 是**本轮三文件范围**
     （e2e 7 ＋ command 2 ＋ lifecycle 0）的合计，不是全量每跑；全量实测 213。
     「HEAD 与修复版完全相同」仍然成立。原文按house规矩保留，勿改。 -->
...
> **⛔ 2026-09-06（第二节）证伪并结项本待办 3（原文保留在上，勿改）**：
> 「每跑 `delta=9`」**缺范围标注**——9 ＝ e2e(7)＋command(2)＋lifecycle(0)，
> 是**那一轮三文件范围内**的合计（本 agent 单跑那三个文件复现：delta ＝ 9，分毫不差）。
> **全量每跑实测 213。** 它**当时是一次正确的测量，只是量错了范围，不是 23 倍的测量错误。**
> 而「每跑」读起来就是全量，9 个目录看着无害，**这就是它被 🟢 归档三轮的原因**。
> 本轮已修（只动 `test/helpers.mjs`，调用点零改动，213 → 0），存量 33.2 万个也已回收。

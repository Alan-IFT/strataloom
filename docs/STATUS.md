...
| 差距 | 影响 | 计划 |
|---|---|---|
| **Coding Memory 缺失** | 工程经验（已验证修复/失败方案/陷阱）只能塞进 `fact`，召回时无法区分 | 阶段 1 |
| **L2 场景层缺失** | 无法「快速恢复一个工作场景」；当前 rollup 是全库压缩，非按场景组织 | 阶段 2 |
| **L3 画像层未独立** | 跨仓画像仅靠 personal scope 的原子偏好，无高层归纳 | 阶段 3 |
| **无向量检索 / RRF** | 仅 FTS5（其 rank 即 BM25）。同义不同词的召回会漏 | 阶段 4 |
| **管线未经真实 LLM 验证** | 全部跑在 fixture adapter 上 | 需真实凭据，待定 |

...
> 最后更新：2026-08-31 · **v0.3.6 已发布并安装，但尚未生效**（schema v9，无迁移）
> **每次工作结束时更新本页**，它是新会话的唯一入口。

## 🔴 阻塞：装上了，但运行中的仍是 0.3.4 —— 需要重启 harness

**0.3.5 与 0.3.6 的全部修复至今未在真实数据上产生一个字节。**
`INSTALL.md` 明写 "Then restart the harness."，而 `c786ad6` 这个提交存在的
全部理由就是记录「安装 ≠ 生效」。**同一个坑第二次**，故写在最顶部。

三条独立证据（均已复现）：

| 证据 | 事实 | 推论 |
|---|---|---|
| **A（决定性）** | global 库 L3 画像 `length(body) = 677`，写于 08-30 17:20 | 0.3.5+ 的 `parsePersona` 强制截到 `PERSONA_TARGET_CHARS = 600`，**677 > 600，写它的不可能是 0.3.5+** |
| **B** | 0.3.6 落盘（08-30 17:47:56）后新写入 147 行 L0，`tool-call:` 行 **0 条** | 采集正常（`tool:*` 74、`assistant` 64），但跑的是旧代码——采集 `tool-call:` 正是 0.3.6 的全部意义 |
| **C** | `dsh web` 进程启于 **2026-08-29 11:20:11** | 早于 0.3.5（08-30 02:16）与 0.3.6（08-30 17:47）两次发布 |

**重启后的验收判据**（下次开工先查这三条）：

1. 新写入的 L0 出现 `tool-call:` 行，孤儿率从 100% 开始下降；
2. **那条 677 字符的画像被重写到 600 以内**——它是「修复未生效」的现场
   物证，也是「已生效」的天然判据（D9 会在下次 personal 写入时删它，
   下一轮维护重建）；
3. 新产生的 L2 场景块 body ≤ `ROLLUP_TARGET_CHARS = 620`。

**在此之前，任何「XX 已修复」的结论都只对代码成立，对运行中的系统不成立。**

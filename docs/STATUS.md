...
> 最后更新：2026-09-02 · **v0.4.7 已发布**：给 `queryInjectionRows` 的**派生
> 分支**补上行数上限（复用 `INJECT_TOP_N`）。**产品行为零变化、落库字节零变化**
> ——9 个真实库经三层逐字节比对确认。本轮结项的是**上一轮登记的待办 j**
> （与 v0.4.4 待办 4 是同一条 LIMIT）。
...
|---|---|---|---|
| k | **`ORDER BY derived DESC, updated_at DESC` 零测试覆盖，而本轮改动放大了它的后果** | 四种变异（`ASC` / 只留 `updated_at` / 去掉整个 `ORDER BY` / `updated_at ASC`）**全部 235 全绿**。关键对照：**加 LIMIT 前**丢掉 layer 优先级只是**重排**（26 行全返回，portrait 仍在，位置 1→26）；**加 LIMIT 后**它变成**静默淘汰**——实测 26 条派生行时 L3 画像**直接掉出 20 行窗口** | **本轮亲手把 `ORDER BY` 从「展示偏好」升级成了「选择谓词」**，而选择谓词无任何执行点看守。今天不可兑现（真实库最多 6 行 < 20，LIMIT 不生效），**随派生行增长而兑现**。补测试很便宜：播新旧两批 + 一条最旧的 PERSONA，断言画像留在包里。已写进 `fts.ts` 注释 |

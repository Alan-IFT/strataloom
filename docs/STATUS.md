...
> 最后更新：2026-08-31 · **v0.3.5 已发布并安装**（schema v9，无迁移）
> **每次工作结束时更新本页**，它是新会话的唯一入口。
> ⚠️ 工作区有**未提交**改动：ADR 0008 的 M1–M8 修复（`src/`+`test/`+`docs/`），
> `npm run verify` **171 passed / 0 fail**（基线 161），版本号未动。
> 已过代码审查（4 项）与 QA（1 阻断 + 2 建议），全部已修：采集路径两处可抛
> 异常（深嵌套 `arguments`、非字符串 `name`）、M8 数字不可复现、值上限理由
> 措辞、转写上限的余量论证（三次下调至 5300）、`sourceOf` 读出口行预算失配。

## ✅ 已修复（未提交）：L0 现在记录 agent 做了什么

...
**那 5 次失败的根因已基本查明：是路由，不是 L3 代码。** 依据（`jobs.payload`
把每次入队时的默认路由固化了下来，等于一份路由变更史）：

- 失败的画像 job 固化的是 `claude/claude-opus-5`（2026-08-23 当天的默认）；
- 该库现在的默认是 `codex/gpt-5.6-sol`，`claude` 已不在配置的 provider 里；
- 排除了最像的两个假设：失败时画像输入只有 **2 条记忆 / 606 字符**，远不触及
  上限，故非 `max-tokens`；提示词 1146 字符且格式良好，故非提示词过长。

且**用已安装的 0.3.0 驱动真实 global 库副本跑通了 L3**：job `done`、恰好写入
1 条画像行、`last_error` 为空。故 L3 代码路径是好的。

这也暴露一个**尚未修复**的设计问题：`llm-call.ts` 的注释写着「登出的固化
provider 不应 5 连败进 dead-letter」，但它只在**当前默认路由与固化路由不同**时
才回落一次。若失败发生时默认路由恰好仍是那个坏路由，回落条件不成立，于是
5 次全败。真正的判据应是「这个 provider 现在还存在吗」，而不是「它和默认是否
相同」。**先观察重启后的真实结果再动手**——若新路由一次成功，说明它不是当前
的活缺陷，不值得为它新增机制。

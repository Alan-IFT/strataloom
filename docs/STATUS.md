...
> 最后更新：2026-09-02 · **v0.4.5 已发布**：派生层写入侧改按渲染后 token 约束。
> **本轮会改变落库字节**——这是它与 v0.4.3 / v0.4.4「产品行为零变化」的关键区别。
> **每次工作结束时更新本页**，它是新会话的唯一入口。
>
> 版本序（**取数时刻 2026-09-02 00:55**）：装机版本 **0.4.2**，插件落盘
> **2026-09-01 17:03:28**，进程启动 **2026-09-01 22:44:18**（PID 1083250）——
> 进程晚于插件，故**跑着的是 0.4.2**。**0.4.3 / 0.4.4 / 0.4.5 三版均已发布、均未安装。**
>
> **要让 0.4.5 生效，需先安装再重启**（`INSTALL.md` 的 remove-then-add，
> 然后重启 harness）。**在此之前不要拿运行时行为验证本轮修复**——
> 新的裁剪只发生在**下一次 rebuild**，而 rebuild 跑的是装机代码。查到
> 「派生行还是老长度」时，结论是**「还没装」或「还没重建」**，不构成证伪。
>
> ⚠️ **本轮读到的一条失真**：上一版本页写「进程启动 17:11:03」，而 22:44
> 已另有一次重启——**判据没变，是数据过期了，而没人回来改**。所以这里记的是
> **取数时刻与 PID**，不只是结论：状态断言必须能被一条命令复核
> （版本序见下节「开工第一件事」，发布状态见 `gh release list`），
> 否则它只是某个时刻的印象。
>

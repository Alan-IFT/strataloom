...
> 最后更新：2026-08-23 · 阶段 2 完成
> **每次工作结束时更新本页**，它是新会话的唯一入口。

## 一句话

插件**可用且已验证**（118 测试全绿，含真平台 e2e 与打包安装契约）。
4×4 架构（D10，不可删减）：**四类 Memory 全部到位；四层中 L0/L1/L2 完成，
仅剩 L3 画像**。差距与分期见 [`design/4x4-memory.md`](design/4x4-memory.md)。

## 下一步

**阶段 3：L3 Core/Persona**。地基已就位——`derived` 已是层级，`LAYER.PERSONA`
（=3）已定义且被 schema 接受，D9 触发器已覆盖全部层级，故**无需再动 schema**。

要做的是：在 `global.sqlite` 中维护**恰一条**画像行，**低频重建**。
注意 L3 与 L2 的关键差异：它跨仓、每用户一条，重建条件不是「预算溢出」而是
「长期趋势变化」——**这个触发条件尚未定义，开工前先定义它**，否则会重蹈
L2/L3 语义未定的老问题（见 ADR 0002）。

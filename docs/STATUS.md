...
> 最后更新：2026-09-01 · **v0.4.3 待发布**（纯文档＋测试，**产品行为零变化**）
> 上一版 v0.4.2 已发布并安装（`sourceOf` 改读引文；schema **不变**，无迁移）
> **每次工作结束时更新本页**，它是新会话的唯一入口。
>
> 版本序：插件落盘 **17:03:28**，进程启动 **11:54:14** ——
> **进程早于插件，所以现在跑着的仍是 0.4.1**。装机产物已按字节核验
> （`truncatedToBudget` / `SOURCE_NOT_SHOWN` 在场、`EXCERPT_COMPACT_MS` 已消失），
> 但**重启前不要拿运行时行为去验证本轮修复**——查不到新行为只说明没生效。
>
> ⚠️ **本轮发现并修正了本页自己那条版本判据命令**（见下节）：它用单个

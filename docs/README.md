...
规范也会错。v2.7 更正了 6 处规范失真（如 D4 原写「只有一个写入口」实为
两个）；v2.8 又更正 3 处，且这次是**真实运行数据**推翻的——规范规定的
`ON CONFLICT DO NOTHING` 配上确定性 job id，使 dead-letter 成了永久否决。
**发现规范错时改规范，不要把代码改回去**，并在 `decisions/` 记一笔。

第四种权威源由此浮现：**真实库里的数据**。测试全绿不代表产品有效——L3 画像
连续五天不可用，而 153 个测试全部通过。定期用 `scripts/inspect.mjs` 看真实
状态，别只看测试。
